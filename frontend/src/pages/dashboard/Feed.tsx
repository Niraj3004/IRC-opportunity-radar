...
export const Feed = () => {
  ...
  const { data: opportunitiesData, isLoading: isLoadingOpp } = useQuery({
    queryKey: ['opportunities', { page, type, q, sort }],
    queryFn: async () => {
      ...
      return res.data.data; // { opportunities: [], total, page, totalPages }
    },
  });
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-8 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4 p-4 bg-surface rounded-lg border border-gray-800">
          ...
          {opportunitiesData?.totalPages > 1 && (
            <div className="flex justify-center gap-2 mt-8 pt-4">
              ...
              <span className="flex items-center px-4 text-sm text-gray-400">
                Page {page} of {opportunitiesData.totalPages}
              </span>
              <Button 
                variant="outline"
                size="sm"
                disabled={page >= opportunitiesData.totalPages}
                onClick={() => handleFilterChange('page', (page + 1).toString())}
              >
                Next
              </Button>
