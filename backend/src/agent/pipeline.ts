...
export const runAgentPipeline = async () => {
  ...
  for (const source of activeSources) {
    ...
    // Fetching state removed since it's not in the enum
    await source.save();
    ...
    try {
      ...
      if (!changed) {
        ...
        continue; // FetchLog already created inside hasSourceChanged
      }
      ...
      await FetchLog.create({
        ...
        itemsNew: itemsNew,
        itemsChanged: 0,
        llmCalls: source.type === 'html' ? 1 : 0, // Simplified count
      });
